...
void physics_collision_handler(body_t *body1, body_t *body2, vector_t axis,
  ...
  if (mass1 == INFINITY) {
    double impulse_scalar = mass2 * (1 + force_const) * (ub - ua);
    vector_t impulse = vec_multiply(impulse_scalar, axis);
    impulse = vec_negate(impulse);
    body_add_impulse(body2, impulse);
    return;
  }
  if (mass2 == INFINITY) {
    double impulse_scalar = mass1 * (1 + force_const) * (ub - ua);
    body_add_impulse(body1, vec_multiply(impulse_scalar, axis));
    return;
  }
  double impulse_scalar =
      (mass1 * mass2) / (mass1 + mass2) * (1 + force_const) * (ub - ua);
  vector_t impulse = vec_multiply(impulse_scalar, axis);
...
force_job_t *forces_job_init(force_creator_t force_creator, void *aux,
                             list_t *bodies) {
  force_job_t *new = malloc(sizeof(force_job_t));
  assert(new);
...
void forces_job_run(force_job_t *force_job) {
  force_job->force_creator(force_job->aux);
}

void forces_job_free(force_job_t *force_job) {
  body_aux_free(force_job->aux);
  free(force_job);
};
